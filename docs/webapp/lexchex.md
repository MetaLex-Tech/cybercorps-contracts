...
before you can invest. The cyberRAISE Marketplace and your
[profile](profile.md) both surface your accreditation status and link here.

Getting a credential is a one-time process; afterwards, any round that
...
1. **Questionnaire** — state whether you are applying as an **individual**
   or a **legal entity**, and complete the SEC Rule 501(a) compliance form.
2. **Evaluation** — connect and verify the wallet addresses that hold your
   assets (you prove control of each with a signature), and LeXcheX values
   the portfolio across them. The thresholds reflect the accreditation
   rules — broadly a net-worth target of around **\$1M for individuals** and
   **\$5M for entities**. Multiple wallets can be combined.
3. **Sign and mint** — sign the LeXcheX agreement from your wallet; LeXcheX
   mints the accredited-investor certificate to that wallet.

## Other routes to accreditation

LeXcheX is one path. As the cyberRAISE Marketplace explains, an investor can
also qualify by **investing above a threshold** (around \$200k for an
...
* Credentials can expire; if a round says yours is no longer valid, re-run
  the process to refresh it.

## Good to know

* **Onchain wealth counts.** LeXcheX exists specifically so assets held in
  your wallet can support accreditation.
* **The credential is verifiable but not revealing** — apps confirm you're
  accredited without seeing your financial details.
