...
2. Choose **Setup profile**. This runs a one-time **Authenticate** step (a
   free Sign-In With Ethereum signature) which creates your profile record.
...
Your profile page is a public-facing card showing your picture, name, an
optional founder title, bio, social links, your **LeXcheX accreditation
status**, and your wallet address and any multisigs.
...
* **Accreditation** — shows your LeXcheX status, with a link out to
  ...
  rounds are not open to you.
* **Socials** — verify your X/Twitter account (via its sign-in) and your
  Telegram (via the Telegram login widget).
* **Website and additional info.**
* **Privacy** — each field has a public/private control, so you decide what
  is shown.
...
multisig that lists you as an owner), and set an expiry date. **Sign
delegation** proposes a Safe transaction — you then complete it from your
Safe's own interface, where your co-signers approve it.
