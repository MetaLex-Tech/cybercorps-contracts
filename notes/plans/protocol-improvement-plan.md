...
**Last audited:** 2026-07-19 against the current worktree. P1 is **implemented locally for new
...
All 25 non-fork Foundry test files pass individually on the final local tree. The monolithic
...
- The first in-printer implementation was rejected after size verification: it made
  `CyberCertPrinter` 28,979 bytes, 4,403 bytes over EIP-170. The external controller is 7,531
  bytes; the final `CyberCertPrinter` is 24,539 bytes (37-byte margin), `IssuanceManager` is
  23,952 bytes (624-byte margin), and the legacy migration implementation also fits at 24,514
  bytes (62-byte margin).
