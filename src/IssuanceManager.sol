...
contract IssuanceManager is BorgAuthACL {
    ...
    function initialize(address _auth, address _CORP, address _CyberCertPrinterImplementation) external initializer {
    ...
    function createCertPrinter(
        ...
        ICyberCertPrinter(newCert).initialize(
            _ledger, _name, _ticker, _certificateUri, address(this), _securityType, _securitySeries
        );
        ...
        returns (uint256)
    {
        ...
        emit CertificateCreated(
            tokenId, certAddress, _details.investmentAmount, _details.issuerUSDValuationAtTimeofInvestment, _details
        );
        return id;
    }
    ...
    function endorseCertificate(
        ...
        Endorsement memory newEndorsement =
            Endorsement(endorser, block.timestamp, signature, address(0), agreementId, address(0), "");
