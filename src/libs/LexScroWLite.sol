...
abstract contract LexScroWLite is Initializable {
    ...
    function createEscrow(
        bytes32 agreementId,
        address counterParty,
        Token[] memory corpAssets,
        Token[] memory buyerAssets
    ) public {
    ...
    function finalizeDeal(bytes32 agreementId, string memory buyerName) public {
        ...
        for (uint256 i = 0; i < deal.buyerAssets.length; i++) {
            if (deal.buyerAssets[i].tokenType == TokenType.ERC20) {
                IERC20(deal.buyerAssets[i].tokenAddress).transferFrom(
                    deal.counterParty, ICyberCorp(CORP).companyPayable(), deal.buyerAssets[i].amount
                );
            } else if (deal.buyerAssets[i].tokenType == TokenType.ERC721) {
                IERC721(deal.buyerAssets[i].tokenAddress).safeTransferFrom(
                    deal.counterParty, ICyberCorp(CORP).companyPayable(), deal.buyerAssets[i].tokenId
                );
            } else if (deal.buyerAssets[i].tokenType == TokenType.ERC1155) {
                IERC1155(deal.buyerAssets[i].tokenAddress).safeTransferFrom(
                    deal.counterParty,
                    ICyberCorp(CORP).companyPayable(),
                    deal.buyerAssets[i].tokenId,
                    deal.buyerAssets[i].amount,
                    ""
                );
            }
        }
        ...
        for (uint256 i = 0; i < deal.corpAssets.length; i++) {
            if (deal.corpAssets[i].tokenType == TokenType.ERC20) {
                ...
                IERC721(deal.corpAssets[i].tokenAddress).safeTransferFrom(
                    address(this), deal.counterParty, deal.corpAssets[i].tokenId
                );
            } else if (deal.corpAssets[i].tokenType == TokenType.ERC1155) {
                IERC1155(deal.corpAssets[i].tokenAddress).safeTransferFrom(
                    address(this), deal.counterParty, deal.corpAssets[i].tokenId, deal.corpAssets[i].amount, ""
                );
            }
        }
    }
    ...
    //receiver erc1155s
    function onERC1155Received(address operator, address from, uint256 tokenId, uint256 amount, bytes calldata data)
        external
        returns (bytes4)
    {
