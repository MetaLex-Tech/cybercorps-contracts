...
contract CyberCertPrinter is Initializable, ERC721EnumerableUpgradeable, UUPSUpgradeable {
    ...
    event CertificateAssigned(
        uint256 indexed tokenId, address indexed investorAddress, string investorName, string issuerName
    );
    ...
    function addEndorsement(uint256 tokenId, Endorsement memory newEndorsement) public {
        ...
        emit CertificateEndorsed(
            ...
            endorsements[tokenId].length - 1,
            block.timestamp
        );
    }

    function endorseAndTransfer(uint256 tokenId, Endorsement memory newEndorsement, address from, address to)
        external
    {
    ...
    function _update(address to, uint256 tokenId, address auth) internal virtual override returns (address) {
        ...
        if (from != address(0) && to != address(0)) {
            ...
            if (address(typeHook) != address(0)) {
                (bool allowed, string memory reason) = typeHook.checkTransferRestriction(from, to, tokenId, "");
            ...
            if (address(globalRestrictionHook) != address(0)) {
                (bool allowed, string memory reason) =
                    globalRestrictionHook.checkTransferRestriction(from, to, tokenId, "");
                if (!allowed) revert TransferRestricted(reason);
            }
                ...
                if (endorsements[tokenId].length > 0) {
                    ...
                    if (endorsement.endorsee == to) {
                        ...
                        emit CertificateAssigned(
                            tokenId, to, endorsement.endorseeName, IIssuanceManager(issuanceManager).companyName()
                        );
