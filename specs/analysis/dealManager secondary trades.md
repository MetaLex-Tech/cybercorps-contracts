...
| any                  | `block.timestamp > validUntil`                | `EXPIRED` (logical)  | No status field change; `acceptOffer()` enforces the offer's `validUntil`, while `finalizeDeal()` enforces the settlement's own `secEscrow.expiry` (acceptance + settlement window), which is decoupled from the offer expiry |
...
| An offer's `certPrinter` must be a printer this SPV's IssuanceManager created and tracks                       | `postOffer()` requires `IssuanceManager.isPrinter(certPrinter)` (authoritative registry check, both sides); `acceptOffer()` reuses `offer.certPrinter`, so a buyer can never pay for a cert minted on a fake/foreign printer                                                                                                                                                                                             |
| Only the cert's legal owner of record may list its units for sale                                              | SELL: `postOffer()` requires `legalOwnerOf(tokenId) == offeror`; BUY: `acceptOffer()` requires `legalOwnerOf(sellerTokenId) == acceptor` — nothing downstream binds the token to the seller, so this guards against selling (and being paid for) someone else's units                                                                                                                                                    |
| A void cert can never back a trade                                                                             | A void cert keeps its registered owner and its units, so the ownership and unit checks alone do not stop it. `postOffer()` (SELL), `acceptOffer()` (both sides) and `finalizeSecondaryTradeAgreement()` each reject a void cert, and `IssuanceManager.secondaryTransfer` rejects it again as the last check before the units move                                                                                        |
| Each settlement's expiry runs from acceptance, not the offer's expiry                                          | `acceptOffer()` stamps both the registry contract deadline and `secEscrow.expiry` with `block.timestamp + settlementWindow` (per-DealManager, default 7d), so a lot accepted just before the offer lapses still gets a full finalize window and cannot be made unfinalizeable by a late acceptance. `acceptedAt` is stamped alongside it, so the closing delay is fixed per lot and a later window change cannot move it |
...
| Total paid for a set of units does not depend on how the offer is split into fills                             | `acceptOffer()` prices each lot off the offer's running total, not the lot alone, so `paymentAccepted` always equals the floored pro-rata amount for the units accepted so far. A per-lot price rounds down on every fill, and a buyer keeps the fractions by never taking the last lot                                                                                                                                  |
| Fee always split: integrator portion + platform portion                                                        | `_finalizeSecondaryEscrow`: `integratorFee + platformFee == totalFee`                                                                                                                                                                                                                                                                                                                                                    |
| Secondary trades carry their own rate, apart from primary issuance                                             | `DealManagerFactory.getSecondaryFeeRatio()` keys on the calling DealManager: it returns that instance's secondary override if MetaLeX set one, else the secondary platform default. Primary issuance reads `getDefaultFeeRatio()` over its own default and override, so the two rates move apart. Only the factory owner sets either                                                                                     |
