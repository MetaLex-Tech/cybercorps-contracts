...
| Each settlement's expiry runs from acceptance, not the offer's expiry                                          | `acceptOffer()` stamps both the registry contract deadline and `secEscrow.expiry` with `block.timestamp + settlementWindow` (per-DealManager, default 7d), so a lot accepted just before the offer lapses still gets a full finalize window and cannot be made unfinalizeable by a late acceptance |
